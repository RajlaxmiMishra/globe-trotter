...
async def seed():
    ...
    async with SessionLocal() as session:
        ...
        for acc in ACCOUNTS:
            ...
            if existing_user:
                user_map[acc["email"]] = existing_user
                print(f"  [skip] User already exists: {acc['email']}")
            else:
                user = User(
                    email=acc["email"],
        ...
        if demo_user:
            trips_check = await session.execute(
                select(Trip).where(Trip.user_id == demo_user.id).limit(1)
            )
            if trips_check.scalar_one_or_none():
                print("  [skip] Demo trips already exist")
            else:
                # Fetch cities for stops
                async def get_city(name):
                    r = await session.execute(select(City).where(City.name == name))
                    return r.scalar_one_or_none()

                paris = await get_city("Paris")
                rome = await get_city("Rome")
                barcelona = await get_city("Barcelona")
                tokyo = await get_city("Tokyo")
                kyoto = await get_city("Kyoto")

                # Trip 1: Europe Summer 2026 (public showcase trip)
                if paris and rome and barcelona:
                    trip1 = Trip(
                        user_id=demo_user.id,
                        name="Europe Summer 2026",
                        description="A dream trip through France, Italy and Spain.",
                        start_date=date(2026, 9, 10),
                        end_date=date(2026, 9, 25),
                        is_public=True,
                        share_slug="europe-summer-2026",
                        budget_threshold=3000,
                    )
                    session.add(trip1)
                    await session.flush()

                    stop1 = Stop(trip_id=trip1.id, city_id=paris.id, start_date=date(2026, 9, 10), end_date=date(2026, 9, 14), order_index=0)
                    stop2 = Stop(trip_id=trip1.id, city_id=rome.id, start_date=date(2026, 9, 15), end_date=date(2026, 9, 19), order_index=1)
                    stop3 = Stop(trip_id=trip1.id, city_id=barcelona.id, start_date=date(2026, 9, 20), end_date=date(2026, 9, 25), order_index=2)
                    session.add_all([stop1, stop2, stop3])
                    print(f"  [+] Demo trip 1: Europe Summer 2026")

                # Trip 2: Japan Cherry Blossoms (upcoming)
                if tokyo and kyoto:
                    trip2 = Trip(
                        user_id=demo_user.id,
                        name="Japan Cherry Blossoms",
                        description="Tokyo and Kyoto in spring.",
                    ...
                    stop4 = Stop(trip_id=trip2.id, city_id=tokyo.id, start_date=date(2027, 3, 25), end_date=date(2027, 3, 31), order_index=0)
                    stop5 = Stop(trip_id=trip2.id, city_id=kyoto.id, start_date=date(2027, 4, 1), end_date=date(2027, 4, 5), order_index=1)
                    session.add_all([stop4, stop5])
                    print(f"  [+] Demo trip 2: Japan Cherry Blossoms")
