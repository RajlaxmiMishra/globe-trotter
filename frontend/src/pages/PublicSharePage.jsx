...
export default function PublicSharePage() {
  ...
  const cities = (trip.stops ?? []).map(s => s.city?.name ?? s.city_name ?? '');
          ...
          <div key={stop.id ?? i} className="card">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {stop.city?.name ?? stop.city_name ?? 'Unknown City'}
              </h3>
              {(stop.start_date ?? stop.arrival_date) && (
                <p className="text-xs text-ink/50 mt-0.5 flex items-center gap-1">
                  <Calendar size={11} />
                  {stop.start_date ?? stop.arrival_date} → {stop.end_date ?? stop.departure_date}
                </p>
              )}
              </div>
            </div>
