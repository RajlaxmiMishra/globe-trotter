    ...
    <div className="card overflow-x-auto">
      <div className="min-w-[560px]">
        <div className="grid grid-cols-7 mb-1">
          {DAYS.map(d => (
            <div key={d} className="text-center text-xs font-semibold text-ink/40 py-2">{d}</div>
          ))}
        </div>
        {weeks.map((week, wi) => (
                  ...
                  {date && (
                    <>
                      ...
                      {events.slice(0, 2).map((e, i) => (
                        <div
                          key={i}
                          className="text-xs rounded px-1.5 py-0.5 mb-0.5 truncate bg-sand/15 text-sand"
                          title={e.name}
                        >
                          {e.name}
                        </div>
                      ...
                      {events.length > 2 && (
                        <div className="text-xs text-ink/40 px-1.5">+{events.length - 2}</div>
                      )}
                    </>
                  )}
                </div>
...
export default function CalendarPage() {
  ...
  // Transform days array into event map: { "2026-09-10": [{name, time, cost}] }
  ...
  days.forEach(day => {
    ...
    if (dayActivities.length > 0) {
      eventMap[dateStr] = dayActivities;
      dayActivities.forEach(a => allEvents.push({ date: dateStr, ...a }));
    }
  });

  // Build calendar grid for the trip's start month
        ...
        <div className="space-y-4">
          ...
          {allEvents.length > 0 && (
            <div className="card space-y-2">
              <h2 className="font-semibold text-sm text-ink mb-3">Scheduled Activities</h2>
              {allEvents.map((e, i) => (
                <div key={i} className="flex items-center gap-3 text-sm">
                  <span className="text-xs text-ink/40 w-24 shrink-0">{e.date}</span>
                  {e.time && <span className="text-xs text-ink/40 w-12 shrink-0">{e.time}</span>}
                  <div className="w-2 h-2 rounded-full bg-sand shrink-0" />
                  <span className="text-ink/70 flex-1 truncate">{e.name}</span>
                  {e.cost != null && e.cost > 0 && (
                    <span className="text-xs font-medium text-mint shrink-0">${Number(e.cost).toFixed(0)}</span>
                  )}
                </div>
              ))}
            </div>
          ...
          {allEvents.length === 0 && !isLoading && (
            <div className="card text-center py-8 text-ink/40 text-sm">
              No activities scheduled yet. Add activities to your stops to see them here.
            </div>
          )}
        </div>
      )}
