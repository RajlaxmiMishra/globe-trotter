...
import { Search, Activity, DollarSign, Tag, X } from 'lucide-react';
    ...
    <div className="card hover:shadow-card-hover transition-all duration-200 animate-slide-up">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {activity.category && (
            <Badge color="sand" className="mt-1">{activity.category}</Badge>
          )}
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="relative">
        ...
        />
        {q && (
          <button onClick={() => setQ('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-ink/30 hover:text-ink">
            <X size={16} />
          </button>
        )}
      </div>
      ...
      {!isLoading && (
        <p className="text-xs text-ink/40">
          {isFetching ? 'Searching…' : activities.length + ' activit' + (activities.length !== 1 ? 'ies' : 'y') + (q ? ' matching "' + q + '"' : '')}
        </p>
      )}
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Activity} title="No activities found" description="Try a different search term." />
      ) : (
